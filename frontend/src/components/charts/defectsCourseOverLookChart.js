...
function drawDefectsCourseOverLookChart(data, domId) {
    setTimeout(() => {
        ...
        let finishedDefects = {};
        for(let p of data){
            for(let d of p.defects){
                let date = moment(moment.utc(d.defectStartTime).toDate()).format("YYYY-MM-DD");
                xAxis.push(date);
                //计入统计
                if(totalDefects[date] == undefined) 
                    ...
                    totalDefects[date] ++;
                let finishDay = moment(moment.utc(d.defectEndTime).toDate()).format("YYYY-MM-DD");
...
export { drawDefectsCourseOverLookChart };