    ...
    <div class="outer">
        ...
        <n-form ref="formRef" :show-label="true">
            ...
            <n-form-item label="创建新标签页 addTabPage(新页面的Key,新页面的标题,是否静默启动,是否需要keep-alive,参数)">
                <n-button @click="tabManager.addTabPage('examineList','新标签页启动的')" type="primary">类似target="_blank"</n-button>
            </n-form-item>
            <n-form-item label="静默创建新标签页 addTabPage(新页面的Key,新页面的标题,是否静默启动,是否需要keep-alive,参数)">
                <n-button @click="tabManager.addTabPage('examineList','悄悄启动的',true)" type="primary">很像某些流氓软件</n-button>
            </n-form-item>
            ...
            <n-form-item label="标签页路由重定向 redirectTo(新页面的Key,新页面的标题,是否需要keep-alive,参数)">
                <n-button style="margin-right:10px;" @click="tabManager.self(panelKey).redirectTo('beautifulPlace','美丽新世界',undefined,{place:0})" type="primary">送我去一个美丽的地方
                </n-button>
            </n-form-item>
