                ...
                >
                    <n-layout-header class="sysLogo">
                        <img
                            ...
                            class="logoImg"
                        />
                        <span v-show="!menuCollapsed">NoBug 项目管理系统</span>
                    </n-layout-header>
                    <n-menu
                        :accordion="true"
                        :collapsed="menuCollapsed"
            ...
            <n-layout id="content">
                ...
                >
                    <div class="nav">
                        <n-space justify="space-between">
                            ...
                            <div class="right">
                                ...
                                <n-dropdown
                                    trigger="hover"
                                    :options="avatarHoverOptions"
                                    @select="handleAvatarDropDownSelect"
                                >
                                    <n-avatar
                                        round
                                        size="small"
                                        :src="systemCache.userPortrait"
                                    />
                                </n-dropdown>
                            </div>
                        </n-space>
                    ...
                    <div class="tabs" v-show="!isPanelFullScreen">
                        <n-config-provider
                            :theme-overrides="themes.tabThemeOverrides"
                        >
                            <n-tabs
                            ...
                            >
                                ...
                                >
                                    <template v-slot:tab>
                                        <n-dropdown
                                            :show="panel.showTagDropdown"
                                            :options="tagHoverOptions"
                                            @select="
                                                handleTagMenuSelect(
                                                    $event,
                                                    panel
...
export default {
    ...
    setup() {
        ...
        let systemCache = localStorage.getItem("nobug-system-cache");
        if (systemCache == undefined) {
            router.push("/login");
        } else {
        ...
        const menuOptions = [
            ...
            // {
            //     label: "前往双创教育运维系统",
            //     key: "goto-dualcreative-maintainance-system",
            //     href: "https://dualcreative.codesocean.top",
            ...
            },
            {
                label: "项目考核",
                key: "projectManage",
                disabled: !hasMenuPermission("projectManage"),
                icon: renderIcon(PersonSharp),
                children: [
                    {
                        label: "项目列表",
                        key: "examineList",
                        disabled: !hasMenuPermission("examineList"),
                    },
                    {
                        label: "项目总览",
                        key: "projectOverlook",
                        disabled: !hasMenuPermission("projectOverlook"),
                    },
                ],
            },
            {
                label: "系统管理",
                key: "systemAdministor",
                disabled: !hasMenuPermission("systemAdministor"),
                icon: renderIcon(SettingsSharp),
                children: [
                    {
                        label: "用户管理",
                        key: "userManage",
                        disabled: !hasMenuPermission("userManage"),
                    },
                    {
                        label: "角色管理",
                        key: "roleManage",
                        disabled: !hasMenuPermission("roleManage"),
                    },
                    {
                        label: "考核方案管理",
                        key: "examineParameter",
                        disabled: !hasMenuPermission("examineParameter"),
                    },
                ],
            },
        ];
        ...
        //通过导航菜单启动新标签页
        function runTabThroughMenu(tabKey) {
        ...
        function logout() {
            localStorage.removeItem("nobug-system-cache");
            router.push("/login");
        }

        function hasMenuPermission(permissionName){
            let permissions = JSON.parse(localStorage.getItem("nobug-system-cache")).permissions;
            for(let item of permissions){
                if(item.permissionType == 1 && item.permissionName == permissionName){
                    return true;
                }
            }
            return false;
